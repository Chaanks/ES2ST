...
class S2U(sb.core.Brain):
    ...
    def on_stage_end(self, stage, stage_loss, epoch):
        ...
        if stage == sb.Stage.TRAIN:
            ...
            current_epoch = self.hparams.epoch_counter.current
            if current_epoch % self.hparams.valid_search_interval == 0:
                stage_stats["wer_st_greedy"] = self.wer_metric_st_greedy.summarize(
                    "error_rate"
                )
...
def dataio_prepare(hparams):
    ...
    def audio_pipeline(path):
        ...
        sig = load_waveform_from_stored_zip(
            full_path.as_posix(), int(byte_offset), int(byte_size)
        )
        return sig
    ...
    def unit_pipeline(clip_id):
        ...
        unit = units_loader.get_embedding_by_clip(int(clip_id))
    ...
    for split in hparams["splits"]:
        datasets[split] = sb.dataio.dataset.DynamicItemDataset.from_json(
            ...
            output_keys=["id", "sig", "duration", "unit_bos", "unit_eos"],
        )
