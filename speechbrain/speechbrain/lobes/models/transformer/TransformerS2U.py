...
class TransformerS2U(TransformerInterface):
    ...
    def forward_decoder_only(self, src, tgt, pad_idx=0):
...
#         # units = [item["unit"][0] for item in batch]                
#         # lengths = [len(item) for item in units]
#         # units = torch.nn.utils.rnn.pad_sequence(
#         #     units, batch_first=True, padding_value=self.padding_idx
#         # )

#         units_eos = [item["unit_eos"] for item in batch]   
#         units_len = [len(item) for item in units_eos]             
#         units_eos = torch.nn.utils.rnn.pad_sequence(
#             units_eos, batch_first=True, padding_value=self.padding_idx
#         )
        
#         units_bos = [item["unit_bos"] for item in batch]                
#         units_bos = torch.nn.utils.rnn.pad_sequence(
#             units_bos, batch_first=True, padding_value=self.padding_idx
#         )
        
#         sigs = [item["sig"] for item in batch]
#         print(sigs[0:2])
#         sigs_len = [len(item) for item in sigs]
#         print(sigs_len)
#         sigs_len = torch.LongTensor(np.array(sigs_len))
#         print(sigs_len)
#         sigs = torch.FloatTensor(np.array(sigs))     
#         sigs = torch.nn.utils.rnn.pad_sequence(
#             sigs, batch_first=True, padding_value=0
#         )
        
#         #mask = feats != self.padding_idx
        
#         return units_bos, units_eos, units_len, (sigs, sigs_len)