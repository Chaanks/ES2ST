    ...
    def transcribe_batch(self, wavs, wav_lens):
        ...
        with torch.no_grad():
            ...
            if isinstance(self.tokenizer, speechbrain.dataio.encoder.CTCTextEncoder):
                ...
                predicted_words = [
                    self.tokenizer.decode_ids(token_seq) for token_seq in predictions
                ]
            else:
...
class WhisperASR(Pretrained):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.hparams.decoder.set_decoder_input_tokens(self.tokenizer.prefix_tokens)
...
class UnitHIFIGAN(Pretrained):
    ...
    def decode_unit(self, unit, f0=None, spk=None, emo=None):
        ...
        with torch.no_grad():
            ...
            waveform = self.infer(
                unit.unsqueeze(0).to(self.device), f0=f0, spk=spk, emo=emo
            )
        return waveform.squeeze(0)
...
class PitchPredictor(Pretrained):
    ...
    def set_f0_processor(self, f0_processor_ckpt):
        ...
        self.hparams.model.setup_f0_stats(
            f0_processor.f0_min,
            f0_processor.f0_max,
            f0_processor.f0_bins,
            f0_processor.speaker_stats,
        )
    ...
    def predict(self, unit, spk_id=None, spk=None, emo=None):
        ...
        with torch.no_grad():
            ...
            if emo is not None:
                emo = emo.unsqueeze(0).to(self.device)
            F0 = self.infer(
                unit.unsqueeze(0).to(self.device), spk_id=spk_id, spk=spk, emo=emo
            )
        return F0.squeeze(0)
...
class EncoderDecoderS2UT(Pretrained):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # self.encoder = self.hparams.wav2vec2
        # self.decoder = self.hparams.model
